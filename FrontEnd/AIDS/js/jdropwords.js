jQuery.fn.jDropWords = function(options) {
  ...
  function getDroppedElement(srcElt) {
    var html = '<div class="' + settings.droppedClass + ' clearfix" rel="' + srcElt.attr('id') + '">' +
      ...
      '<div class="action"><a class="close">x</a></div>' +
      '</div>';
    var container = $(html);
    ...
    $(".blank").removeClass("error");
    $('.blanks p .blank').removeClass('error');
    return container;
  }
  ...
  function checkAnswers(answers, appContainer) {
    var score = 0;
    var nbQuestions = 0;
    var animUpDuration = 0;
    var animDownDuration = 0;
    ...
    $('.blanks p .blank', appContainer).each(function() {
      ...
      if (dropped.length) {
        ...
        dropped
          ...
          .animate({'top': posTop + "px"}, {duration : 0, queue : true});
  ...
  function displayScore(appContainer, score, nbQst) {
   ...
   /appContainer.prepend(scoreElt);
    ...
    alertify.alert("Você acertou " + score + " de " + nbQst + " respostas.")
        .set('modal', true)
